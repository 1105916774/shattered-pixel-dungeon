...
public abstract class Mob extends Char {
	...
	protected void onAdd(){
		if (firstAdded) {
			// 修改日志为英文
			GLog.i("\n%s generated at position %d\n", Mob.this.getClass().getSimpleName(), pos);
	...
	protected boolean act() {
		...
		if (justAlerted){
			sprite.showAlert();
			// 修改日志为英文
			GLog.i("\n%s entered alert state\n", Mob.this.getClass().getSimpleName());
		} else {
			sprite.hideAlert();
			sprite.hideLost();
		}
	...
	public boolean add( Buff buff ) {
		if (super.add( buff )) {
			if (buff instanceof Amok || buff instanceof AllyBuff) {
				state = HUNTING;
				// 修改日志为英文
				GLog.i("\n%s state changed to HUNTING (affected by %s)\n", Mob.this.getClass().getSimpleName(), buff.getClass().getSimpleName());
			} else if (buff instanceof Terror || buff instanceof Dread) {
				state = FLEEING; 
				// 修改日志为英文
				GLog.i("\n%s state changed to FLEEING (affected by %s)\n", Mob.this.getClass().getSimpleName(), buff.getClass().getSimpleName());
			} else if (buff instanceof Sleep) {
				state = SLEEPING;
				// 修改日志为英文
				GLog.i("\n%s state changed to SLEEPING (affected by %s)\n", Mob.this.getClass().getSimpleName(), buff.getClass().getSimpleName());
				postpone(Sleep.SWS);
			}
			return true;
		}
		...
		if (super.remove( buff )) {
			if (state == FLEEING && ((buff instanceof Terror && buff(Dread.class) == null)
				...
				if (enemySeen) {
					...
					state = HUNTING;
					// 修改日志为英文
					GLog.i("\n%s fear ended, entered HUNTING state\n", Mob.this.getClass().getSimpleName());
				} else {
					state = WANDERING;
					// 修改日志为英文
					GLog.i("\n%s fear ended, entered WANDERING state\n", Mob.this.getClass().getSimpleName());
				}
			}
			return true;
		}
	...
	public void onAttackComplete() {
		...
		if (enemy instanceof Hero) {
			GLog.i("\n%s attacked the hero, causing %d damage", this.getClass().getSimpleName(), damageRoll());
		}
		attack( enemy );
		Invisibility.dispel(this);
		spend( attackDelay() );
	...
	protected class Sleeping implements AiState {
		...
		protected void awaken( boolean enemyInFOV ){
			if (enemyInFOV) {
				...
				target = enemy.pos;
				// 修改日志为英文
				GLog.i("\n%s woke up and entered HUNTING state\n", Mob.this.getClass().getSimpleName());
			} else {
				notice();
				state = WANDERING;
				target = Dungeon.level.randomDestination( Mob.this );
				// 修改日志为英文
				GLog.i("\n%s woke up and entered WANDERING state\n", Mob.this.getClass().getSimpleName());
			}
	...
	protected class Wandering implements AiState {
		...
		protected boolean noticeEnemy(){
			...
			target = enemy.pos;
			// 修改日志为英文
			GLog.i("\n%s noticed the enemy and entered HUNTING state\n", Mob.this.getClass().getSimpleName());
		...
		protected void nowhereToRun() {
			if (buff( Terror.class ) == null && buff( Dread.class ) == null) {
				if (enemySeen) {
					...
					state = HUNTING;
					// 修改日志为英文
					GLog.i("\n%s has nowhere to run and entered HUNTING state\n", Mob.this.getClass().getSimpleName());
				} else {
					state = WANDERING; 
					// 修改日志为英文
					GLog.i("\n%s has nowhere to run and entered WANDERING state\n", Mob.this.getClass().getSimpleName());
				}
			}
		}
	}
