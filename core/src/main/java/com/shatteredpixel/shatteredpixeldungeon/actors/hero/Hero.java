...
//		belongings.backpack.items.add(healingPotion); // 添加初始治疗药水到背包

		//-------------------------
		// 添加初始治疗药水并放入快捷栏
		PotionOfHealing healingPotion = new PotionOfHealing();
		healingPotion.collect(belongings.backpack); // 添加初始治疗药水到背包
		Dungeon.quickslot.setSlot(2, healingPotion); // 放入第3个快捷栏位
		//-------------------------
		...
		//--------- 策略+工厂模式 ---------
		// 通过工厂创建默认策略集合
		this.healthCheckContext = new HealthCheckContext(
				HealthCheckStrategyFactory.createDefaultStrategies()
		);
		//-------------------------------
	}

//	//-------------------------
//	private boolean healingSuggested = false;
//	// 血量<50% 时，需要治疗
//	private void checkHealingSuggestion() {
////		GLog.w("Warning: 生命值低！考虑使用治疗药水。");
//		if (HP < 10 && hasHealingPotion()) {
//			healingSuggested = true;
//			hasHealingPotion();
//			GLog.w("Warning: 生命值低！考虑使用治疗药水。\n");
////			GLog.w("Warning: Low health! Using healing potion.\n");
////			GameScene.flashForHealingPotion();
//		} else {
//			healingSuggested = false;
//		}
//	}
//	// 判断背包是否存在治疗药水
...
//	//-------------------------
	...
	private final HealthCheckContext healthCheckContext; // 健康检查上下文
	private void checkHealingSuggestion() { // 检查治疗建议
		healthCheckContext.checkAndExecute(this);
	}
	//-----------------------------------------------
	...
	public boolean act() {
		//-------------------------
		super.act();
		checkHealingSuggestion(); // 每个行动周期检测血量
		//-------------------------
